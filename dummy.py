...
class node:
    ...
    def poller_server(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            while 1:
                conn, addr = sock.accept()
                voter_client_thread = threading.Thread(target=self.handle_polling_response, args=(conn, addr), daemon=False)
                voter_client_thread.start()
    """
    def handle_polling_response(self, conn, polling_respondent):
    ...
    def conduct_polling(self, sus_ID):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            print("Starting poll collecter server at node ", self.ID)
            sock.bind(('localhost', ports_vote_collection[self.ID]))
            sock.listen()
            sock.settimeout(30)  # To avoid waiting forever

            # collect polling responses until all nodes that were requested reply back
            while set(polling_respondents) != set(polling_requests):
                try:
                    conn, addr = sock.accept()
                ...
                polling_response_thread = threading.Thread(target=self.handle_polling_response,
                                                           args=(conn, polling_respondent),
                                                           daemon=True)
